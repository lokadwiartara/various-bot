<?php
	...
	function openBrowserSaveSession(){
		...
		$options->addArguments(array('--start-maximized', '--disable-infobars', '--disable-gpu', '--no-sandbox' )); // ,'--headless'
	...
	echo "------------------------------------\n";
	echo "Mengumpulkan buyer dari ".$target."...\n" ; 
	echo "------------------------------------\n";
	$driver->get('https://m.bukalapak.com/u/'.$target.'/feedback');
	sleep(rand(5,7));
	...
	if(count($getListLinkBuyer) > 0){
		...
		foreach($linkBuyers as $linkBuyer){
			$no = $x + 1;
			echo $x.". "." Proses mem-Follow up buyer ".$nameBuyers[$x]." => ".$linkBuyer." ... ";
			...
			if(checkExistElement($driver,$patternInputTextChat = '#new_messages_message > div.message-chat-textarea.js-has-hint > textarea')){
				...
				$pesan = '{Halo|Salam|Assalamualaikum} '.$nameBuyers[$x].', {mohon maaf|maaf} {mengganggu|ganggu} waktunya. {Hanya|Sekedar} ingin {menginfokan|memberikan informasi} produk yang bermanfaat,  {kemungkinan} Anda sedang {mencarinya|membutuhkannya}. ' ; 
				
				$pesanSpin = $spintax->process($pesan);

				$inputTextChat = $driver->findElement(WebDriverBy::cssSelector($patternInputTextChat));

				$inputTextChat->sendKeys($pesanSpin);
