<?php
	...
	if($iconProfile == NULL){ // setelah dari sini langsung ke $btnLocation di baris ke 124 
		
		
		// mulai dari sini dulu aja 
		...
		// sampai sini terakhir ... setelah itu baru bagian profil nya ke atas ... 
	...
	if(count($iconProfile) > 0){
		...
		foreach($iconProfile as $item => $a){
			...
			if($existBtnShowNo != NULL){
				...
				$db->where ('contact_name', $nama);
				$db->where ('contact_number', $noWA);
				$results = $db->get ('tbl_contact');
				if(count($results) == 0){
					$data = Array (
					    "contact_name" => $nama,
					    "contact_number" => $noWA
