<?php
	...
	switch($task){
		...
		case 2: 
			echo "Masukkan nomor handphonenya tanpa tanda + di depannya, \nGunakan format 628xxxxx : ";
			...
			echo "Masukkan pesan yang akan dikirimkan (ketikan 0 jika ingin keluar): \n\n";
			...
			$driver->get('https://api.whatsapp.com/send?phone='.$noHP.'&text='.$pesan);
...
// console.log(JSON.stringify(window.WAPI.sendMessageToID("3EB063AF88176A2F2F00", "tostos")));

// console.log(JSON.stringify(window.WAPI.getMyContacts()));

// console.log(JSON.stringify(window.WAPI.getContact("6283811575876@c.us")));

window.WAPI.sendMessageToID("'.trim($noHP).'@c.us", "'.trim($pesan).'");

';

// echo $noHP. ' ' .$pesan ;

			$driver->executeScript($tesJS);
			break;
